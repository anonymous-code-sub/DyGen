...
def load_dataset(args, dataset):
    ...
    if dataset == '20news':
        ...
        # newsgroups_train  = fetch_20newsgroups(data_home='./data/20news', subset='train',  shuffle=True, random_state=args.seed)
        newsgroups_train  = fetch_20newsgroups(data_home=args.path, subset='train',  shuffle=True, random_state=args.seed)
        print(newsgroups_train.target_names)
        print(len(newsgroups_train.data))

        # newsgroups_test  = fetch_20newsgroups(data_home='./data/20news', subset='test',  shuffle=False)
        ...
        train_df = pd.read_csv('./data/agnews/train.csv', header=None)
        ...
        test_df = pd.read_csv('./data/agnews/test.csv', header=None)
...
def create_dataset(args):
    if args.dataset == '20news' or args.dataset == 'agnews' or args.dataset == 'wos':
        ...
        if args.dataset == 'trec':
            ...
            num_labels = 9
            args.num_classes = 9
        args.folder_path = './data/{}'.format(args.dataset)
        if args.saved_dataset == 'n':
...
def main():
    ...
    train_inputs, train_masks, train_labels, noisy_train_labels, validation_inputs, validation_masks, validation_labels, noisy_validation_labels, test_inputs, test_masks, test_labels = read_data(args, num_labels)
    train_sampler, train_dataloader = create_dataset(args, train_inputs, train_masks, train_labels, args.train_batch_size, 'train', noisy_train_labels)
    validation_sampler, validation_dataloader = create_dataset(args, validation_inputs, validation_masks, validation_labels, args.eval_batch_size, 'eval', noisy_validation_labels)
    test_sampler, test_dataloader = create_dataset(args, test_inputs, test_masks, test_labels, args.eval_batch_size, 'test')

    number_points = int((len(train_inputs)+len(validation_inputs)) * args.noise_ratio / args.num_iter)
