...
def train_CR(args, num_points, num_labels, train_sampler, train_dataloader, validation_sampler, validation_dataloader, prediction_sampler, prediction_dataloader):
    ...
    for epoch in trange(args.epochs, desc="Epoch"): 
      # Training
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                for idx in range(len(outputs)):
                    ...
                    # print(outputs[idx][-1][-1].shape, outputs[idx][-1][-1][:,0,:].shape)
        ...
        # print(len(train_embeds), train_embeds[0].shape)
        for idx in range(len(outputs)):
    ...
    # Put model in evaluation mode
        ...
        if args.dataset != 'wos' and args.dataset != 'tacred':
            ...
            for idx in range(len(model.models)):
                ...
                if args.detect_model == 'euclidean':
                ...
                elif args.detect_model == 'mahalanobis':
                    dists = mahalanobis_dist(args, dists_embeds, dists_labels)
                    full_dists[idx] = dists
                    dists = [dists[i][dists_labels[i]] for i in range(len(dists))]
                    dists_epochs.append(dists)
                elif args.detect_model == 'relative_mahalanobis':
                    dists = relative_mahalanobis_dist(args, dists_embeds, dists_labels)
                    full_dists[idx] = dists
                    dists = [dists[i][dist_labels[i]] for i in range(len(dists))]
                    dists_epochs.append(dists)
    ...
    # return dists_list[:, :train_embeds.shape[0]], dists_list[:, train_embeds.shape[0]:], test_embeds_list, best_model

def train_base(args, num_points, num_labels, train_sampler, train_dataloader, validation_sampler, validation_dataloader, prediction_sampler, prediction_dataloader):
    model = BertForSequenceClassification.from_pretrained(args.bert, num_labels=num_labels, output_hidden_states=True)
    if torch.cuda.device_count() > 1:
        print("Let's use", torch.cuda.device_count(), "GPUs!")
        model = nn.DataParallel(model)
    ...
    # trange is a tqdm wrapper around the normal python range
    dists_epochs = []
    train_embeds_list = None
    eval_embeds_list = None
    test_embeds_list = None
    dists_list = None
    for epoch in trange(args.epochs, desc="Epoch"): 
      # Training
        # Set our model to training mode (as opposed to evaluation mode)
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                if train_embeds == None:
                    train_embeds = outputs[-1][-1][:,0,:].squeeze()
                    train_labels = b_labels
                    train_logits = F.softmax(outputs[0], dim=-1)
                else:
                    train_embeds = torch.cat((train_embeds, outputs[-1][-1][:,0,:].squeeze()), 0)
                    train_labels = torch.cat((train_labels, b_labels), 0)
                    train_logits = torch.cat((train_logits, F.softmax(outputs[0], dim=-1)), 0)

        if train_embeds_list == None:
            train_embeds_list = torch.zeros((args.epochs, train_embeds.shape[0], train_embeds.shape[1]))
            train_embeds_list[0] = train_embeds.detach()
        else:
            train_embeds_list[epoch] = train_embeds.detach()
        # Tracking variables 
        eval_accurate_nb = 0
        nb_eval_examples = 0
        logits_list = []
        ...
        for batch in validation_dataloader:
            ...
            # Forward pass, calculate logit predictions
                outputs = model(b_input_ids, token_type_ids=None, attention_mask=b_input_mask) 
                if eval_embeds == None:
                    eval_embeds = outputs[-1][-1][:,0,:].squeeze()
                    eval_labels = b_labels
                    eval_logits = F.softmax(outputs[0], dim=-1)
                else:
                    eval_embeds = torch.cat((eval_embeds, outputs[-1][-1][:,0,:].squeeze()), 0)
                    eval_labels = torch.cat((eval_labels, b_labels), 0)
                    eval_logits = torch.cat((eval_logits, F.softmax(outputs[0], dim=-1)), 0)
                logits = outputs[0]
                logits_list.append(logits)
                labels_list.append(b_labels)
            # Move logits and labels to CPU
            ...
            eval_accurate_nb += tmp_eval_nb
            nb_eval_examples += label_ids.shape[0]
        if eval_embeds_list == None:
            eval_embeds_list = torch.zeros((args.epochs, eval_embeds.shape[0], eval_embeds.shape[1]))
            eval_embeds_list[0] = eval_embeds.detach()
        else:
            eval_embeds_list[epoch] = eval_embeds.detach()
        eval_accuracy = eval_accurate_nb/nb_eval_examples
        print("Validation Accuracy: {}".format(eval_accuracy))
        scheduler.step(eval_accuracy)
    ...
    # Put model in evaluation mode
        ...
        for batch in prediction_dataloader:
            ...
            with torch.no_grad():
                ...
                logits = outputs[0]
                if test_embeds == None:
                    test_embeds = outputs[-1][-1][:,0,:].squeeze()
                    test_labels = b_labels
                else:
                    test_embeds = torch.cat((test_embeds, outputs[-1][-1][:,0,:].squeeze()), 0)
                    test_labels = torch.cat((test_labels, b_labels), 0)
                logits_list.append(logits)
                labels_list.append(b_labels)
            # Move logits and labels to CPU
            logits = logits.detach().cpu().numpy()
            ...
            eval_accurate_nb += tmp_eval_nb
            nb_test_examples += label_ids.shape[0]
        if test_embeds_list == None:
            test_embeds_list = torch.zeros((args.epochs, test_embeds.shape[0], test_embeds.shape[1]))
            test_embeds_list[0] = test_embeds.detach()
        else:
            test_embeds_list[epoch] = test_embeds.detach()

        print("Test Accuracy: {}".format(eval_accurate_nb/nb_test_examples))
    
        dists_embeds = torch.cat((train_embeds, eval_embeds, test_embeds), 0).cpu()
        dists_labels = torch.cat((train_labels, eval_labels, test_labels), 0).cpu()
        if args.detect_model == 'euclidean':
            dists = euclidean_dist(args, dists_embeds, dists_labels)
            full_dists = dists
            dists = [dists[i][dists_labels[i]] for i in range(len(dists))]
        ...
        if dists_list == None:
            dists_list = torch.zeros((args.epochs, full_dists.shape[0], full_dists.shape[1]))
            dists_list[0] = full_dists.detach()
        else:
            dists_list[epoch] = full_dists.detach()
